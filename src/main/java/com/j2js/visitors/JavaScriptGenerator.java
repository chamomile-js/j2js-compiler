...
public class JavaScriptGenerator extends Generator {
	...
	/**
	 * A try statement has the following structure: TryStatement : try Block
	 * Catch* (Catch | Finally)
	 *
	 * Because ECMAScript3 does not support multiple catch clauses, we will
	 * generate the following code. TryStatement : try Block catch(ex) { Catch }
	 * Finally? try Block catch(ex) { if (ex ...) { Catch } else if(ex ...) ...
	...
	public void visit(TryStatement tryStmt) {
		...
		// Note: We have to declare variable 'ex_' because we cannot use method
		// local variables due to
		// a JavaScript bug in some clients: var l0 = "foo"; try { ... } catch
		// (l0)
		// { /* Now l0 == "foo" */ }
		String ex = null;
		if (clause != null) {
	...
	public void visit(InfixExpression binOp) {
		 ...
		 * truncate
		 * the result. Note that the % operation works the same in ECMAScript
		 * and
		 * Java, i.e. in accepting floating-points.
		 */
   ...
   @formatter:on */
	...
	private void generateScriptCode(MethodInvocation invocation) {
		...
		if (name.equals("put")) {
			// In the HTML document object model the window property of the
			...
			print(firstArg); // TODO error handling
		} else {
	...
	private static final String SCRIPT_ENGINE_CLASS = "javax.script.ScriptEngine";
	private static final String SCRIPT_HELPER_CLASS = "javascript.ScriptHelper";
	...
	public void visit(MethodInvocation invocation) {
		...
		if (className.equals(SCRIPT_HELPER_CLASS)) {
			generateScriptCode(invocation);
			return;
		}

		if (className.equals(SCRIPT_ENGINE_CLASS) && (name.equals("put") || name.equals("eval"))) {
			generateScriptCode(invocation);
			return;
		}
		...
		if (className.startsWith("j2js.w3c.dom.")/*
			...
			if (name.equals("setInterval")) {
				// Example: createTimerDelegate(this, listener, delayInMillis,
				// 'Interval');
			...
			// TreeWalker
			if (name.equals("createTreeWalker")) {
				print(prefix + "createTreeWalker(");
				expression.visit(this);
				print(", ");
				generateList(invocation.getArguments());
				print(")");
	...
	public void visit(CastExpression cast) {
		if (false && cast.getTypeBinding() != Type.VOID) { // &&
															// J2JSCompiler.compiler.isCheckCast())
															// {
