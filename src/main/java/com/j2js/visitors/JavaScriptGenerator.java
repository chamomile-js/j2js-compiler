...
public class JavaScriptGenerator extends Generator {
  ...
  public void visit(TryStatement tryStmt) {
    ...
    // a JavaScript bug in some clients: var l0 = "foo"; try { ... } catch (l0)
    // { /* Now l0 == "foo" */ }
    String ex = null;
    if (clause != null) {
  ...
  public void visit(InfixExpression binOp) {
    ...
    /*
     * There is no integral type division in ECMAScript, so we need to truncate
     * the result. Note that the % operation works the same in ECMAScript and
  ...
  public void visit(MethodInvocation invocation) {
    ...
    if (className.startsWith("j2js.w3c.dom.")/*
      ...
      if (isW3C(invocation)) {
        ...
        // if (methodBinding.equals("j2js.w3c.dom.html.HTMLFrameElement") &&
